...
plt.subplot(122)
plt.plot(time[t_index_fourth] - time[t_index_fourth][0], phi_errors_real[t_index_fourth,0] / L, 'r', label=r"$F_1(\Phi(t,\xi_0)_3) - r_1(t;\pi(\xi_0))$")
plt.plot(time[t_index_fourth] - time[t_index_fourth][0], phi_errors_real[t_index_fourth,1] / L, 'b', label=r"$F_2(\Phi(t,\xi_0)_3)-r_2(t;\pi(\xi_0))$")
#plt.plot(time[t_index_first], np.abs(pos_y[t_index_first] -mollifiers_values[] ))
plt.ylim([-10,10])
max_error = 3
plt.plot([time[t_index_fourth][0], time[t_index_fourth][-1]] - time[t_index_fourth][0], max_error *np.ones(2), 'k--')
plt.plot([time[t_index_fourth][0], time[t_index_fourth][-1]] - time[t_index_fourth][0], -max_error *np.ones(2), 'k--')
plt.plot(time[t_index_fourth] - time[t_index_fourth][0], gps_acc_pos[t_index_fourth], 'g', label=r"GNSS acc")
plt.xlabel(r"Time $(s)$")
plt.ylabel(r"Errors $(m)$")
plt.title("GNSS and trajectory errors \n $\\varepsilon_1=\\varepsilon_2=1$")
plt.legend()
