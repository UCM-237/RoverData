...
data_rover = pd.read_table(r"25_12_03__10_05_14:2.csv")
...
#%% Get the epsilons
index_eps_points = (data_points_real[:,0]==0) # Booleans
...
for k in range(len(epsilons)):
    if(k==0):
        plt.plot(mollifiers_values[:,1,k], mollifiers_values[:,0,k], '--', label=r"$F_{\varepsilon}$")
    else:
...
plt.plot(time[t_index_first] - time[t_index_first][0], np.sqrt(errors_first[:,0]**2 + errors_first[:,1]**2), 'b', 
         label=r"dist$(r(t;r_0), \mathcal{P})$")
